...
class CNodeStateRelease : EbState
{
    ...
    public CNodeStateRelease(CNode node)
    {
        ...
        _defState("CNodeStateRelease", "Fsm", 0, false);
    }

    //-------------------------------------------------------------------------
    public override void enter()
    {
        EbLog.Note("CNodeStateRelease.enter() NodeType=" + mNode.getNodeType() + " NodeId=" + mNode.getNodeId());
            ...
            if (exit_all)
            {
                ...
                foreach (int i in groups.Keys)
                {
                    ...
                    if (exit_successor_id != null)
                    {
                        ...
                        if (e_id > 0) mNode.getNodeMgr()._opCreateNode(e_id, _eNodeState.Init, mNode.getNodeId());
                    }
                }
            }
            else
    ...
    {
        EbLog.Note("CNodeStateRelease.exit() NodeType=" + mNode.getNodeType() + " NodeId=" + mNode.getNodeId());
    }
}